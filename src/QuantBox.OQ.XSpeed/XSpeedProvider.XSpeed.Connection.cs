...
namespace QuantBox.OQ.XSpeed
{
    partial class XSpeedProvider
    {
        ...
        private void _Connect()
        {
            ...
            //XSpeedAPI.GetInstance().__RegDepthMarketDataDictionary(_dictDepthMarketData);
        ...
        private void Connect_MsgQueue()
        {
            ...
            lock (_lockMsgQueue)
            {
                ...
                {
                    m_pMsgQueue = CommApi.XSpeed_CreateMsgQueue();

                    CommApi.XSpeed_RegOnConnect(m_pMsgQueue, _fnOnConnect_Holder);
                    CommApi.XSpeed_RegOnDisconnect(m_pMsgQueue, _fnOnDisconnect_Holder);
                    CommApi.XSpeed_RegOnRspError(m_pMsgQueue, _fnOnRspError_Holder);

                    CommApi.XSpeed_StartMsgQueue(m_pMsgQueue);
                }
            }
        }
        ...
        private void _Disconnect()
        {
            ...
            //XSpeedAPI.GetInstance().__RegDepthMarketDataDictionary(null);
        ...
        private void Disconnect_MsgQueue()
        {
            lock (_lockMsgQueue)
            {
                if (null != m_pMsgQueue && IntPtr.Zero != m_pMsgQueue)
                {
                    CommApi.XSpeed_StopMsgQueue(m_pMsgQueue);

                    CommApi.XSpeed_ReleaseMsgQueue(m_pMsgQueue);
        ...
        #region 连接状态回调
        private void OnConnect(IntPtr pApi, ref DFITCUserLoginInfoRtnField pRspUserLogin, ConnectionStatus result)
        {
            //用于行情记算时简化时间解码
            //try
            //{
            ...
            if (m_pMdApi == pApi)//行情
            {
                ...
                if (ConnectionStatus.E_logined == result)
                {
                    ...
                    mdlog.Info("accountID:{0},loginResult:{1},SHFETime:{2},DCETime:{3},CZCETime:{4},FFEXTime:{5},lRequestID:{6},sessionID:{7}",
                        pRspUserLogin.accountID, pRspUserLogin.loginResult, pRspUserLogin.SHFETime,
                        pRspUserLogin.DCEtime, pRspUserLogin.CZCETime, pRspUserLogin.CFFEXTime,
                        pRspUserLogin.lRequestID, pRspUserLogin.sessionID);
            ...
            else if (m_pTdApi == pApi)//交易
            {
                ...
                if (ConnectionStatus.E_logined == result)
                {
                    ...
                    tdlog.Info("accountID:{0},loginResult:{1},SHFETime:{2},DCETime:{3},CZCETime:{4},FFEXTime:{5},lRequestID:{6},sessionID:{7}",
                        pRspUserLogin.accountID, pRspUserLogin.loginResult, pRspUserLogin.SHFETime,
                        pRspUserLogin.DCEtime, pRspUserLogin.CZCETime, pRspUserLogin.CFFEXTime,
                        pRspUserLogin.lRequestID, pRspUserLogin.sessionID);
                ...
                //{
                    ...
                    //_dictCommissionRate.Clear();
                    //_dictMarginRate.Clear();
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.CFFEX, DFITCInstrumentTypeType.COMM_TYPE);
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.CZCE, DFITCInstrumentTypeType.COMM_TYPE);
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.DCE, DFITCInstrumentTypeType.COMM_TYPE);
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.SHFE, DFITCInstrumentTypeType.COMM_TYPE);

                    TraderApi.TD_ReqQryArbitrageInstrument(m_pTdApi, DFITCExchangeIDType.DCE);
                    TraderApi.TD_ReqQryArbitrageInstrument(m_pTdApi, DFITCExchangeIDType.CZCE);

                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.DCE, DFITCInstrumentTypeType.OPT_TYPE);
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.CFFEX, DFITCInstrumentTypeType.OPT_TYPE);
                    TraderApi.TD_ReqQryExchangeInstrument(m_pTdApi, DFITCExchangeIDType.CZCE, DFITCInstrumentTypeType.OPT_TYPE);
